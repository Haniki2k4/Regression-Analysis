---
title: "Báo Cáo Phân Tích Môi Trường Sống và Sức Khỏe Hộ Gia Đình"
author: "Nhóm 5"
date: "`r Sys.Date()`"
output:
  html_document:
...
library(knitr)
library(kableExtra)
library(MASS)       # Chứa các hàm và bộ dữ liệu cho thống kê ứng dụng
...
Bộ dữ liệu được lấy từ khảo sát `MICS6 (2021)`, `MICS5 (2013 -14)`, `MICS4 (2010 -11)`, `MICS3 (2006)`, `MICS2 (2000)` của UNICEF tại Việt Nam, bao gồm các thông tin về hộ gia đình, trẻ em và các yếu tố liên quan đến sức khỏe cộng đồng.
...
# Định nghĩa hàm đọc dữ liệu từ file hộ gia đình (hh.sav)
...
# Định nghĩa hàm đọc dữ liệu từ file trẻ em (ch.sav)
...
```{r descriptives}
# Bảng tần số cho biến nước sạch 
...
# Bảng tần số cho biến nhà tiêu hợp vệ sinh
...
# Chuẩn bị dữ liệu cho biểu đồ nước sạch
safe_tab <- hh6_clean %>% 
...
# Vẽ biểu đồ tròn tỷ lệ hộ sử dụng nước sạch bằng ggplot2 
...
# Chuẩn bị dữ liệu cho biểu đồ nhà tiêu 
san_tab <- hh6_clean %>% 
...
# Vẽ biểu đồ tròn tỷ lệ nhà tiêu hợp vệ sinh bằng ggplot2 
...
# Nhà tiêu hợp vệ sinh theo khu vực
...
```{r chi-square}
# Thực hiện kiểm định Chi-bình phương (Chi-squared) 
chisq_res <- chisq.test(hh6_clean$safe_water, hh6_clean$any_diarrhea)

# Hiển thị bảng chéo để xem tần suất thực tế bằng knitr
...
```{r regression}
# Chuyển các biến phân loại sang dạng factor với nhãn rõ ràng
hh6_clean <- hh6_clean %>%
  mutate(
    windex5_factor = factor(windex5, labels = c("_Nghèo_nhất", "_Nhóm_2", "_Nhóm_3", "_Nhóm_4", "Giàu nhất")),
    windex5r_factor = factor(windex5r, labels = c("_Nghèo_nhất", "_Nhóm_2", "_Nhóm_3", "_Nhóm_4", "Giàu nhất")),
    windex5u_factor = factor(windex5u, labels = c("_Nghèo_nhất", "_Nhóm_2", "_Nhóm_3", "_Nhóm_4", "Giàu nhất")),
    helevel_factor = factor(helevel, labels = c("_Không_bằng_cấp", "_Tiểu_học", "_THCS", "_THPT", "_Trung cấp", "_Cao_đẳng/Đại_học", "_Không_biết")) 
  )

# Chạy mô hình logistic tổng quát
model1 <- glm(improved_sanitation ~ windex5_factor + helevel_factor + HH6 + HC2,
             data = hh6_clean,
             family = binomial)
...
```{r regression_r}
# Chạy mô hình logistic cho khu vực nông thôn
model2 <- glm(improved_sanitation ~ windex5r_factor + helevel_factor + HC2,
             data = hh6_clean,
             family = binomial)
...
```{r regression_u}
# Chạy mô hình logistic cho khu vực nông thôn
model3 <- glm(improved_sanitation ~ windex5u_factor + helevel_factor + HC2,
             data = hh6_clean,
             family = binomial)
...
# Gộp tất cả các bộ dữ liệu đã được làm sạch vào một dataframe duy nhất
...
-   Tuy nhiên, năm 2013-2014 sụt giảm đột ngột nhưng sau đó phục hồi lại lên gần 100% vào năm 2021. 
**Lý do** cho vấn đề sụt giảm đột ngột này là vì sự thay đổi trong tiêu chuẩn xét nguồn nước sạch, không công nhận nước đóng chai phù hợp để uống.
...
-   Cũng có sự giảm mạnh năm 2013-2014 xuống gần 80% và phục hồi rõ rệt lên khoảng 95% vào năm 2021.
